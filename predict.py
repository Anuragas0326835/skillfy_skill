...
def load_latest_model(experiment_name):
    ...
    """
    # DagsHub connection is already configured
    ...
    try:
        ...
        if runs.empty:
            print(f"No finished runs found in experiment '{experiment_name}'. Please run the training script.")
            return None
...
if __name__ == "__main__":
    # Initialize DagsHub and set up remote MLflow tracking
    # Replace <YOUR_DAGSHUB_USERNAME> and <YOUR_REPO_NAME> with your actual details
    dagshub.init(repo_owner='Anuragas0326835', repo_name='Dagshub_demo', mlflow=True)


    # Load the latest model from MLflow
    model_pipeline = load_latest_model(EXPERIMENT_NAME)
